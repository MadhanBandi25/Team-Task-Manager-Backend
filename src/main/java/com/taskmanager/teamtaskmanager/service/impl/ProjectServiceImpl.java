...
public class ProjectServiceImpl implements ProjectService {
    ...
    public ProjectResponse createProject(ProjectRequest request, Long userId) {
        ...
        boolean exists = projectRepository.findByIsDeletedFalse()
                .stream()
                .anyMatch(project -> normalizeName(project.getName()).equals(newProjectName));

        if (exists) {
            throw new DuplicateResourceException("Project already exists with name: " + request.getName());
        }
