...
public class TaskServiceImpl implements TaskService {
    ...
    public TaskResponse createTask(TaskRequest request, Long createdById) {
        ...
        User loggedInUser = userRepository.findByEmailAndIsDeletedFalse(loggedInEmail)
                .orElseThrow(() -> new ResourceNotFoundException("Logged in user not found"));

        if (!loggedInUser.getId().equals(createdById)) {
            throw new AccessDeniedException("You cannot create task for another user");
        }
        ...
        boolean taskExists = taskRepository.findByProjectIdAndIsDeletedFalse(request.getProjectId())
                .stream()
                .anyMatch(task ->
                        normalizeText(task.getTitle()).equals(newTaskTitle));

        if (taskExists) {
            throw new DuplicateResourceException(
                    "Task already exists with title: " + request.getTitle());
        }


        User assignedUser = userRepository.findByIdAndIsDeletedFalse(request.getAssignedToUserId())
